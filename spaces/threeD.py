...
class State(Surface):
    ...
    def run_gd(self, epsilon=0.01, alpha=0.9, nesterov=False):
        """Runs Gradient Descent algorithm with momentum and
        returns the parameter update history.
        ...
        nesterov -- whether the momentum is nesterov momentum or
        standard momentum (default False)
    ...
    def run_adam(
                self, epsilon=0.001, rho1=0.9, rho2=0.999,
                alpha=0.9, delta=1.e-8
    ):
    ...
    def plot_steps(
        self, iters, steps_until_n=50,
        azimuth=10, elevation=55, color_map=cm.gray,
        ...
        colors -- color for each type of optimizer
        ...
        for i in iters:
            ...
            ax.scatter(x_history[-n_back:],
                       y_history[-n_back:],
                       z_history[-n_back:],
                       c=colors[loop_counter],
                       marker="o", alpha=1, s=point_sizes)
            loop_counter += 1
